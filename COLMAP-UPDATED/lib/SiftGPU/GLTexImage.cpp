...
void GLTexImage::DrawQuad() {
    ...
    glTexCoord2i(0, 0);
    glVertex2i(0, 0);
    glTexCoord2i(0, _drawHeight);
    glVertex2i(0, _drawHeight);
    glTexCoord2i(_drawWidth, _drawHeight);
    glVertex2i(_drawWidth, _drawHeight);
    glTexCoord2i(_drawWidth, 0);
...
void GLTexImage::ZeroHistoMargin(int width, int height) {
    ...
    if (marginx > 0 || marginy > 0) {
        ...
        if (right > width && _texWidth > width) {
            ...
            glVertex2i(width, 0);
            glTexCoord2i(width, bottom);
            glVertex2i(width, bottom);
...
void GLTexImage::DrawMargin(int right, int bottom) {
    ...
    if (right > _drawWidth) {
        glTexCoord2i(_drawWidth, 0);
        glVertex2i(_drawWidth, 0);
        glTexCoord2i(_drawWidth, bottom);
        glVertex2i(_drawWidth, bottom);
        glTexCoord2i(right, bottom);
        glVertex2i(right, bottom);
        glTexCoord2i(right, 0);
        glVertex2i(right, 0);
...
void GLTexPacked::DrawMargin(int right, int bottom, int mx, int my) {
    ...
    if (right > tw && mx) {
        glTexCoord2i(tw, 0);
        glVertex2i(tw, 0);
        glTexCoord2i(tw, bottom);
        glVertex2i(tw, bottom);
        glTexCoord2i(right, bottom);
        glVertex2i(right, bottom);
        glTexCoord2i(right, 0);
        glVertex2i(right, 0);
...
int
GLTexInput::DownSamplePixelDataF(unsigned int gl_format, int width, int height, int ds, const float *pin, float *pout,
                                 int skip) {
    int step, linestep;
    int i, j;
    int ws = width / ds - skip;
    ...
    if (GlobalUtil::_UseCUDA || GlobalUtil::_UseOpenCL) {
        ...
        if (!simple_format) {
            ...
            if (gl_type == GL_UNSIGNED_BYTE)
                ...
                DownSamplePixelDataF(gl_format, width, height, 1 << _down_sampled, (float *) data, _converted_data,
                                     skip);
        ...
        if (simple_format && (_down_sampled > 0 || (gl_format != GL_LUMINANCE && GlobalUtil::_PreProcessOnCPU))) {
            ...
            if (gl_type == GL_UNSIGNED_BYTE) {
                ...
                DownSamplePixelDataI(gl_format, width, height, 1 << _down_sampled, ((const unsigned char *) data),
                                     newdata);
                ...
                DownSamplePixelDataI(gl_format, width, height, 1 << _down_sampled, ((const unsigned short *) data),
                                     newdata);
