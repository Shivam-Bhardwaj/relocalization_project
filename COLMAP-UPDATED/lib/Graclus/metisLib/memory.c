...
void AllocateWorkSpace(CtrlType *ctrl, GraphType *graph, int nparts) {
    ...
    if (ctrl->optype == OP_KMETIS) {
        ctrl->wspace.edegrees = (EDegreeType *) GKmalloc(graph->nedges * sizeof(EDegreeType),
                                                         "AllocateWorkSpace: edegrees");
        ...
        */
        ctrl->wspace.maxcore = 3 * (graph->nvtxs + 1) +                 /* Match/Refinement vectors */
                               ...
                               graph->nvtxs * (sizeof(ListNodeType) / sizeof(idxtype)) +
                               /* Greedy k-way balance/refine */
                               20  /* padding for 64 bit machines */
                ;
    } else if (ctrl->optype == OP_KVMETIS) {
        ctrl->wspace.edegrees = NULL;
        ctrl->wspace.vedegrees = (VEDegreeType *) GKmalloc(graph->nedges * sizeof(VEDegreeType),
        ...
        ctrl->wspace.maxcore = 5 * (graph->nvtxs + 1) +                 /* Refinement vectors */
                               ...
                               2 * graph->ncon * graph->nvtxs * (sizeof(ListNodeType) / sizeof(idxtype)) +
                               /* 2-way refinement */
...
void FreeWorkSpace(CtrlType *ctrl, GraphType *graph) {
    GKfree((void **) &ctrl->wspace.edegrees, (void **) &ctrl->wspace.vedegrees, (void **) &ctrl->wspace.core,
           (void **) &ctrl->wspace.pmat, LTERM);
}
...
idxtype *idxwspacemalloc(CtrlType *ctrl, int n) {
    ...
    ctrl->wspace.ccore += n;ASSERT(ctrl->wspace.ccore <= ctrl->wspace.maxcore);
...
void idxwspacefree(CtrlType *ctrl, int n) {
    ...
    ctrl->wspace.ccore -= n;ASSERT(ctrl->wspace.ccore >= 0);
}
