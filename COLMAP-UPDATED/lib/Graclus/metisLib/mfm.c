...
void MocFM_2WayEdgeRefine(CtrlType *ctrl, GraphType *graph, float *tpwgts, int npasses) {
    ...
    if (ctrl->dbglvl & DBG_REFINE) {
        ...
        printf("] T[%.3f %.3f], Nv-Nb[%5d, %5d]. ICut: %6d, LB: %.3f\n", tpwgts[0], tpwgts[1], graph->nvtxs,
               graph->nbnd, graph->mincut, origbal);
    }
    ...
    for (pass = 0; pass < npasses; pass++) { /* Do a number of passes */
        ...
        ASSERT(ComputeCut(graph, where) == graph->mincut);ASSERT(CheckBnd(graph));

        /* Insert boundary nodes in the priority queues */
        nbnd = graph->nbnd;
        RandomPermute(nbnd, perm, 1);
        for (ii = 0; ii < nbnd; ii++) {
            i = bndind[perm[ii]];ASSERT(ed[i] > 0 || id[i] == 0);ASSERT(bndptr[i] != -1);
        ...
        for (nswaps = 0; nswaps < nvtxs; nswaps++) {
            ...
            if (ctrl->dbglvl & DBG_MOVEINFO) {
                printf("Moved %6d from %d(%d). Gain: %5d, Cut: %5d, NPwgts: ", higain, from, cnum,
