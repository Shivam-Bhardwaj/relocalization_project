...
void MocInit2WayBalance2(CtrlType *ctrl, GraphType *graph, float *tpwgts, float *ubvec) {
    ...
    if (ctrl->dbglvl & DBG_REFINE) {
        ...
        printf("] T[%.3f %.3f], Nv-Nb[%5d, %5d]. ICut: %6d, LB: %.3f [B]\n", tpwgts[0], tpwgts[1], graph->nvtxs,
               graph->nbnd, graph->mincut, ComputeLoadImbalance(ncon, 2, npwgts, tpwgts));
    }
    ...
    ASSERT(ComputeCut(graph, where) == graph->mincut);ASSERT(CheckBnd(graph));ASSERT(CheckGraph(graph));
