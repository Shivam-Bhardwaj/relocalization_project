...
**************************************************************************/
void
Random_KWayEdgeRefineMConn(CtrlType *ctrl, GraphType *graph, int nparts, float *tpwgts, float ubfactor, int npasses,
                           int ffactor) {
    ...
    tvwgt = idxsum(nparts, pwgts);ASSERT(tvwgt == idxsum(nvtxs, graph->vwgt));
    ...
    for (pass = 0; pass < npasses; pass++) { ASSERT(ComputeCut(graph, where) == graph->mincut);
        ...
        for (nmoves = iii = 0; iii < graph->nbnd; iii++) {
            ...
            if (myrinfo->ed >= myrinfo->id) { /* Total ED is too high */
                ...
                if (myedegrees[k].ed - myrinfo->id > 0)
                    ...
                    if (/*(iii&7) == 0  ||*/ phtable[myedegrees[k].pid] == 2 || pwgts[from] >= maxwgt[from] ||
                ...
                *======================================================================*/
                ...
                for (j = xadj[i]; j < xadj[i + 1]; j++) {
                    ...
                    ASSERT(myrinfo->ndegrees <= xadj[ii + 1] - xadj[ii]);ASSERT(CheckRInfo(myrinfo));
...
**************************************************************************/
void
Greedy_KWayEdgeBalanceMConn(CtrlType *ctrl, GraphType *graph, int nparts, float *tpwgts, float ubfactor, int npasses) {
...
void EliminateComponents(CtrlType *ctrl, GraphType *graph, int nparts, float *tpwgts, float ubfactor) {
    ...
    while (nleft > 0) {
        if (first == last) { /* Find another starting vertex */
            cptr[++ncmps] = first;ASSERT(touched[todo[0]] == 0);
            i = todo[0];
            cind[last++] = i;
            touched[i] = 1;
            me = where[i];
