                ...
                for (j = k + 1; j < myndegrees; j++) {
                    ...
                    if ((myedegrees[j].ed > myedegrees[k].ed &&
                        ...
                        (myedegrees[j].ed == myedegrees[k].ed &&
                         IsHBalanceBetterTT(ncon, nparts, npwgts + myedegrees[k].pid * ncon, npwgts + to * ncon, nvwgt,
                                            ubvec)))
                        k = j;
                }
...
void MCGreedy_KWayEdgeBalanceHorizontal(CtrlType *ctrl, GraphType *graph, int nparts,
    ...
    for (pass = 0; pass < npasses; pass++) { ASSERT(ComputeCut(graph, where) == graph->mincut);
        ...
        for (;;) {
            ...
            for (j = k + 1; j < myndegrees; j++) {
                to = myedegrees[j].pid;
                if (IsHBalanceBetterTT(ncon, nparts, npwgts + myedegrees[k].pid * ncon, npwgts + to * ncon, nvwgt,
                                       ubvec))
                    k = j;
            }
