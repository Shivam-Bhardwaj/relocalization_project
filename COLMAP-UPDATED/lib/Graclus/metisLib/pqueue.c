...
void PQueueInit(CtrlType *ctrl, PQueueType *queue, int maxnodes, int maxgain) {
    ...
    if (queue->type == 1) {
        ...
        if (WspaceAvail(ctrl) > ncore) {
            ...
            queue->nodes = (ListNodeType *) idxmalloc((sizeof(ListNodeType) / sizeof(idxtype)) * maxnodes,
                                                      "PQueueInit: queue->nodes");
            queue->buckets = (ListNodeType **) idxmalloc((sizeof(ListNodeType *) / sizeof(idxtype)) * j,
...
int PQueueInsert(PQueueType *queue, int node, int gain) {
    ...
    if (queue->type == 1) { ASSERT(gain >= -queue->ngainspan && gain <= queue->pgainspan);
...
int PQueueDelete(PQueueType *queue, int node, int gain) {
    ...
    if (queue->type == 1) { ASSERT(gain >= -queue->ngainspan && gain <= queue->pgainspan);ASSERT(queue->nnodes > 0);
        ...
        ASSERT(locator[node] != -1);ASSERT(heap[locator[node]].val == node);
...
int PQueueUpdate(PQueueType *queue, int node, int oldgain, int newgain) {
    ...
    if (queue->type == 1) {
        ...
        ASSERT(locator[node] != -1);ASSERT(heap[locator[node]].val == node);ASSERT(
                heap[locator[node]].key == oldgain);ASSERT(CheckHeap(queue));
...
void PQueueUpdateUp(PQueueType *queue, int node, int oldgain, int newgain) {
    ...
    if (queue->type == 1) { ASSERT(oldgain >= -queue->ngainspan && oldgain <= queue->pgainspan);ASSERT(
                newgain >= -queue->ngainspan && newgain <= queue->pgainspan);ASSERT(queue->nnodes > 0);
...
int CheckHeap(PQueueType *queue) {
    ...
    for (i = 1; i < nnodes; i++) { ASSERTP(locator[heap[i].val] == i,
                                           ("%d %d %d %d\n", nnodes, i, heap[i].val, locator[heap[i].val]));ASSERTP(
                heap[i].key <= heap[(i - 1) / 2].key,
