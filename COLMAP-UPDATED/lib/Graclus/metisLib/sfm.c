...
void FM_2WayNodeRefine(CtrlType *ctrl, GraphType *graph, float ubfactor, int npasses) {
    ...
    for (pass = 0; pass < npasses; pass++) {
        ...
        for (ii = 0; ii < nbnd; ii++) {
            i = bndind[perm[ii]];ASSERT(where[i] == 2);
            PQueueInsert(&parts[0], i, vwgt[i] - rinfo[i].edegrees[1]);
            PQueueInsert(&parts[1], i, vwgt[i] - rinfo[i].edegrees[0]);
        }

        ASSERT(CheckNodeBnd(graph, nbnd));ASSERT(CheckNodePartitionParams(graph));
        ...
        for (nswaps = 0; nswaps < nvtxs; nswaps++) {
            ...
            ***********************************************************/
            ...
            IFSET(ctrl->dbglvl, DBG_MOVEINFO,
                  printf("Moved %6d to %3d, Gain: %5d [%5d] [%4d %4d] \t[%5d %5d %5d]\n", higain, to, g[to], g[other],
                         vwgt[u[to]], vwgt[u[other]], pwgts[0], pwgts[1], pwgts[2]));
    ...
    for (pass = 0; pass < npasses; pass++) {
            ...
            ***********************************************************/
            ...
            IFSET(ctrl->dbglvl, DBG_MOVEINFO,
                  printf("Moved %6d to %3d, Gain: %5d [%5d] \t[%5d %5d %5d] [%3d %2d]\n",
                         higain, to, (vwgt[higain] - rinfo[higain].edegrees[other]), vwgt[higain], pwgts[0], pwgts[1],
                         pwgts[2], nswaps, limit));
        ...
        *****************************************************************/
        for (nswaps--; nswaps > mincutorder; nswaps--) {
            ...
            ASSERT(CheckNodePartitionParams(graph));ASSERT(where[higain] == to);
    ...
    for (nswaps = 0; nswaps < nvtxs; nswaps++) {
        ...
        IFSET(ctrl->dbglvl, DBG_MOVEINFO,
              printf("Moved %6d to %3d, Gain: %3d, \t[%5d %5d %5d]\n", higain, to,
                     vwgt[higain] - rinfo[higain].edegrees[other], pwgts[0], pwgts[1], pwgts[2]));
