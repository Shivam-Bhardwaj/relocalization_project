...
void
pingpong(CtrlType *ctrl, GraphType *graph, int nparts, int chain_length, float *tpwgts, float ubfactor, int toplevel)
...
int
local_search(CtrlType *ctrl, GraphType *graph, int nparts, int chain_length, idxtype *w, float *tpwgts, float ubfactor)
                ...
                if (sum[me] > w[j]) // if this cluster where j belongs is not a singleton
                    for (k = 0; k < nparts; k++)
                        if (k != me) {
                            ...
                            tempchange = -kDist[ii][me] * sum[me] * w[j] / (sum[me] - w[j]) +
                                         kDist[ii][k] * sum[k] * w[j] / (sum[k] + w[j]);
...
float onePoint_move(GraphType *graph, int nparts, idxtype *sum, idxtype *squared_sum, idxtype *w, idxtype *self_sim,
    ...
    if (sum[me] > w[s]) { // if this cluster where j belongs is not a singleton
        ...
        for (k = 0; k < me; k++) {
            ...
            if (sum[k] > 0)
                tempchange = squared_sum[me] * inv_sum1 + squared_sum[k] * 1.0 / sum[k] - q1 * inv_sum2 -
                             q2 * 1.0 / (sum[k] + w[s]);
...
void MLKKMRefine(CtrlType *ctrl, GraphType *orggraph, GraphType *graph, int nparts, int chain_length, float *tpwgts,
                 float ubfactor) {
