...
void Weighted_kernel_k_means(CtrlType *ctrl, GraphType *graph, int nparts, idxtype *w, float *m_adjwgt, float *tpwgts,
                             float ubfactor)
...
int
local_search(CtrlType *ctrl, GraphType *graph, int nparts, int chain_length, idxtype *w, float *m_adjwgt, float *tpwgts,
             float ubfactor)
        ...
        for (j = 0; j < nvtxs; j++)
            if (mark[j] == 0) {
                ...
                if (sum[me] > w[j]) // if this cluster where j belongs is not a singleton
                    for (k = 0; k < nparts; k++)
                        if (k != me) {
                            tempchange = -sum[me] * w[j] / (sum[me] - w[j]) * kDist[j][me] +
                                         sum[k] * w[j] / (sum[k] + w[j]) * kDist[j][k];
